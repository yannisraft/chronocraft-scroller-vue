...
<div class="scroller-container" :style="[{ height: height + 'px' }]">
    <div :id="scrollerid" :class="[
        ...
        { height: height + 'px' },
        {
          ...
          'overflow-y': GetOverflowY() + ' !important'
        },
      ]">
        <div :class="[
          'scroller-content',
          ...
          { 'animate-next': animatedirection === 1 },
          { 'animate-previous': animatedirection === -1 }
        ]" :style="[{ gap: gap + 'px' }, { padding: contentpadding + 'px' }]">
            <slot name="content">
                <div v-for="datacell in cellsdata" :id="'cell_' + datacell.id" ref="cellRef" :key="datacell.id" :class="[
              'scroller-cell',
              orientation === 'vertical' ? 'vertical-cell' : 'horizontal-cell',
              datacell.debug ? 'debugcellstyle' : ''
            ]" :style="{'flex-basis': cellFlexBasis, height: cellH, width: cellW, 'z-index': datacell.index}">
...
export default defineComponent({
    ...
    setup(props, context) {
        ...
        let scrollerid =
            "scroller_" + (Math.floor(Math.random() * 999999) + 1000000);
        ...
        function GeneratePreviousData(newdata) {
            ...
            // Remove Last n Cells where n=newdata.length
            //
            cellsdata.value.splice(
                cellsdata.value.length - newdata.length,
                cellsdata.value.length - 1
            );
            ...
            if (sign === 1) {
                if (props.orientation === "vertical") {
                    ...
                    if (
                        ...
                        scroller.scrollWidth - scrollLoadingOffset ||
        ...
        function GetTotalVisibleCells() {
            ...
            if (cell) {
                ...
                if (props.orientation === "vertical") {
                    ...
                    totalVisibleCells = parseInt(
                        Math.round(totalCellsAtY) * props.numcols
                    );
                } else {
                    ...
                    totalVisibleCells = parseInt(
                        Math.round(totalCellsAtX) * props.numrows
                    );
                }

                var minimumRequiredInitialCells =
                    totalVisibleCells + 3 * props.newcellslength;
                ...
                if (!props.static) {
                    if (minimumRequiredInitialCells > cellsdata.value.length) {
                        console.log(
                            "Error: Minimum Cells Required to load at start should be: ",
                            minimumRequiredInitialCells
                        );
                        ...
                        alerttext.value =
                            "Error: Minimum Cells Required to load at start should be: " +
                            minimumRequiredInitialCells +
                            ". This value is relevant to the scroller width and to the number of new cells added each time.";
                    }
                }
            }
        }
                ...
                if (cellsdata.value[k]) {
                    if (cellsdata.value[k].id === id) {
                        ...
                        if (props.orientation === "vertical") {
                            ...
                            foundcell_position =
                                cellelement.offsetTop - contentpadding + props.gap;
                        } else {
                            var contentpadding = parseInt(content.style.paddingRight);
                            foundcell_position = parseInt(
                                cellboundingrect.left - props.gap - contentpadding
                            );
                        }
                        break;
                    }
        ...
        function Initialize() {
            ...
            if (props.orientation === "vertical") {
                ...
                newCellW =
                    (scroller.offsetWidth - totalgap - props.contentpadding * 2) /
                    props.numcols;
                ...
                if (props.cellsquared) {
                    newCellH =
                        (scroller.clientWidth - totalgap - props.contentpadding * 2) /
                        props.numcols;
                ...
                newCellH =
                    (scroller.offsetHeight - totalgap - props.contentpadding * 2) /
                    props.numrows;

                //var newCellFlexBasis = ((100 - totalgappercent) / props.numrows);
                var newCellFlexBasis =
                    (scroller.offsetHeight - totalgap - props.contentpadding * 2) /
                    props.numrows;
